...
def set_rules(world: "PokemonXYWorld", player: int) -> None:
    ...
    # Shalour -> Coumarine requires Rumble Badge (Badge 3) + Surf.
    # Route 12 is the road out of Shalour and cannot be crossed without Surf, so
    # everything from Coumarine onwards is behind it. Reported from play testing:
    # "everything on route 12 and beyond requires Surf".
    #
    # This slightly over-restricts one location, "Route 12 - Found Honey
    # (recurring)", which sits before the water. Over-restricting is the safe
    # direction -- it can only make logic more conservative, never unbeatable.
    get_entrance("Shalour Area -> Coumarine Area").access_rule = \
        lambda state: state.has("Rumble Badge", player) and state.has("HM03 Surf", player)

    # Coumarine -> Lumiose requires Plant Badge (Badge 4) ONLY.
    #
    # The Power Plant Pass used to be required here, which was wrong twice over.
    # Route 13 lives in this region and needs no pass to enter, and the pass's own
    # vanilla location IS on Route 13 -- so requiring it to get in made the item
    # gate the region it is found in. The pass now gates the Power Plant itself
    # (via `requires` in Locations.py) and onward progression below.
    get_entrance("Coumarine Area -> Lumiose Area").access_rule = \
        lambda state: state.has("Plant Badge", player)

    # Lumiose -> Laverre requires Voltage Badge (Badge 5) + Power Plant Pass.
    # Progression past Lumiose runs through the Power Plant, so the pass is
    # needed here rather than on the way in.
    get_entrance("Lumiose Area -> Laverre Area").access_rule = \
        lambda state: state.has("Voltage Badge", player) and state.has("Power Plant Pass", player)
