...
# Per-location corrections, for places where a single route straddles two tiers
# and a prefix rule would be wrong for part of it.
#
# Route 7 is the clearest case: the Snorlax blocks it partway, so the far side
# needs the Poke Flute (Cyllage tier) but the near side is reachable as soon as
# you can walk out of Camphrier. These two sit on the near side.
REGION_OVERRIDES_EXACT = {
    "Route 7 - Miracle Seed item ball disappeared": "Camphrier Area",
    "Route 7 - Heal Ball item ball disappeared": "Camphrier Area",
}


def _resolve_region(loc_name: str, loc_data, regions_dict) -> str:
    # Exact names win over prefixes, so a single location can opt out of its
    # route's blanket rule.
    exact = REGION_OVERRIDES_EXACT.get(loc_name)
    if exact and exact in regions_dict:
        return exact
