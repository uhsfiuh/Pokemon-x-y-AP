...
-- Location: Z:\ProgramData\Archipelago\data\lua\pokemon_xy_connector.lua
--
-- One connector for both games. It was previously split into
-- pokemon_x_connector.lua and pokemon_y_connector.lua, which were identical
-- apart from a fallback string that is only ever used when BizHawk cannot
-- report the ROM hash itself.
...
-- ============================================================================
-- CONNECTOR DIAGNOSTICS  (`diag` in the command box)
--
-- For the case where everything looks healthy -- socket bound, client connected,
-- no errors -- but no check ever fires. The usual cause is that this build's
-- `mainmemory` domain does not reach the addresses this connector uses. BizHawk
-- returns 0 for an out-of-range read rather than raising, so every event flag
-- reads as clear forever and nothing looks wrong anywhere.
--
-- This prints enough to tell that apart from a genuinely wrong base address.
-- ============================================================================
local function connector_diagnostics()
    ...
    print("=============== CONNECTOR DIAGNOSTICS ===============")
    print("Lua version      : " .. tostring(_VERSION))
    -- The BizHawk build matters more than anything else here: FCRAM layout is
    -- not stable across core revisions, so two people on different builds get
    -- different addresses for the same game.
    print("BizHawk version  : " .. tostring(try(function()
        return emuclient and emuclient.getversion()
    end)))
    print("System id        : " .. tostring(try(function() return emu.getsystemid() end)))
    print("ROM name         : " .. tostring(try(function() return gameinfo.getromname() end)))
    print("ROM hash         : " .. tostring(try(function() return gameinfo.getromhash() end)))
    -- Extra identity fields. getromhash() returns "N/A" for some dumps and a real
    -- hash for others, and it is not yet understood why. getstatus() in
    -- particular says whether BizHawk matched this file in its game database,
    -- which is the most likely explanation.
    print("ROM db status    : " .. tostring(try(function() return gameinfo.getstatus() end)))
    print("ROM in database  : " .. tostring(try(function() return not gameinfo.isstatusbad() end)))
    print("Board type       : " .. tostring(try(function() return gameinfo.getboardtype() end)))
    print("ROM options      : " .. tostring(try(function() return gameinfo.getoptions() end)))
    print("Memory domain    : " .. tostring(try(function() return mainmemory.getname() end)))

    local size = try(function() return mainmemory.getcurrentmemorydomainsize() end)
    if size then
        print(string.format("Domain size      : %d bytes (0x%X)", size, size))
    else
        print("Domain size      : unavailable on this build")
    end

    local domains = try(function() return memory.getmemorydomainlist() end)
    if type(domains) == "table" then
        local names = {}
        for _, n in ipairs(domains) do table.insert(names, tostring(n)) end
        print("Domains present  : " .. table.concat(names, ", "))
    end

    local last_flag_byte = EVENT_FLAGS_BASE + MONITORED_BYTES - 1
    print(string.format("Flag block       : 0x%08X .. 0x%08X", EVENT_FLAGS_BASE, last_flag_byte))
    print(string.format("Badge address    : 0x%08X", BADGE_ADDRESS))
    print(string.format("Bag pocket 1     : 0x%08X", POCKETS[1].addr))

    if size then
        if last_flag_byte >= size then
            print("*** PROBLEM: the flag block sits OUTSIDE this memory domain.")
            print("*** Every read returns 0, so no check can ever be detected.")
            print("*** The addresses in this connector do not match your build.")
        else
            print("Flag block fits inside the domain. Addresses are at least plausible.")
        end
    end

    -- The decisive test: on any real save, plenty of event flags are set.
    local blk = read_block(EVENT_FLAGS_BASE, MONITORED_BYTES)
    local set_bits, nonzero_bytes, examples = 0, 0, {}
    for i = 0, MONITORED_BYTES - 1 do
        local b = blk[i] or 0
        if b ~= 0 then nonzero_bytes = nonzero_bytes + 1 end
        for bit = 0, 7 do
            if math.floor(b / BADGE_MASK[bit]) % 2 == 1 then
                set_bits = set_bits + 1
                if #examples < 6 then
                    local fid = (i * 8) + bit
                    table.insert(examples, string.format("0x%04X%s", fid,
                        LOCATION_NAMES[fid] and " (known location)" or ""))
                end
            end
        end
    end

    print(string.format("Event flags set  : %d bits, across %d non-zero bytes of %d",
        set_bits, nonzero_bytes, MONITORED_BYTES))

    -- The eight gym leader flags, called out individually. These are the flags
    -- that ARE the badge location checks, and they are the only checks reported
    -- as not firing while ordinary trainer checks work. Showing their live state
    -- turns "the badge check did not send" into a yes/no: beat a leader, press
    -- this, and see whether the game actually set the flag.
    local GYM_CHECK_FLAGS = {
        { 0x06D2, "Bug     (Santalune / Viola)"   },
        { 0x0718, "Cliff   (Cyllage / Grant)"     },
        { 0x06E1, "Rumble  (Shalour / Korrina)"   },
        { 0x06E2, "Plant   (Coumarine / Ramos)"   },
        { 0x06E3, "Voltage (Lumiose / Clemont)"   },
        { 0x06E4, "Fairy   (Laverre / Valerie)"   },
        { 0x06E5, "Psychic (Anistar / Olympia)"   },
        { 0x06E6, "Iceberg (Snowbelle / Wulfric)" },
    }
    print("--- Gym leader flags (these ARE the badge location checks) ---")
    for _, g in ipairs(GYM_CHECK_FLAGS) do
        local fid  = g[1]
        local byte = blk[math.floor(fid / 8)] or 0
        local on   = math.floor(byte / BADGE_MASK[fid % 8]) % 2 == 1
        print(string.format("  0x%04X  %s  %s", fid, on and "SET  " or "clear", g[2]))
    end

    if set_bits == 0 then
        print("*** PROBLEM: not one event flag is set anywhere in the block.")
        print("*** On any real save many flags are always set, so either these")
        print("*** addresses are wrong for your build, or the reads return 0.")
        print("*** Compare `Domain size` above against the flag block range.")
    else
        print("First flags set  : " .. table.concat(examples, ", "))
        print("Flags are being read. If checks still do not arrive, the fault is")
        print("on the Archipelago client side, not in this connector.")
    end
    print("=====================================================")
end

-- ============================================================================
-- `flag <id> [on|off]` -- read or write any event flag by hand.
--
-- An investigation tool. Several things in this game turn out not to be gated by
-- what is in your bag, so the only way to find out which flag actually controls
-- something is to toggle it and look. Reports the flag if no on/off is given.
--
-- Writes straight to the event flag block, so anything the Archipelago client
-- watches will react to it. Setting a flag that happens to be a location check
-- WILL send that check.
-- ============================================================================
local function flag_command(arg)
    local idtok, state = (arg or ""):match("^(%S+)%s*(%S*)$")
    if not idtok then
        print(">>> [FLAG] Usage:  flag 0x277        report it")
        print(">>>                flag 0x277 off    clear it")
        print(">>>                flag 0x277 on     set it")
        return
    end

    local fid = tonumber(idtok)
    if not fid then fid = tonumber((idtok:gsub("^0[xX]", "")), 16) end
    if not fid then
        print(">>> [FLAG] Could not read a flag id from: " .. tostring(idtok))
        return
    end

    if fid < 0 or fid >= MONITORED_BYTES * 8 then
        print(string.format(">>> [FLAG] 0x%04X is outside the scanned range (0x0000 - 0x%04X).",
            fid, MONITORED_BYTES * 8 - 1))
        return
    end

    local addr = EVENT_FLAGS_BASE + math.floor(fid / 8)
    local mask = BADGE_MASK[fid % 8]
    local byte = mainmemory.read_u8(addr)
    local on   = math.floor(byte / mask) % 2 == 1
    local name = LOCATION_NAMES[fid]

    if state == "" then
        print(string.format(">>> [FLAG] 0x%04X is %s%s", fid, on and "SET" or "clear",
            name and ("   -- " .. name) or ""))
        return
    end

    local want = (state:lower() == "on" or state == "1" or state:lower() == "set")
    if want == on then
        print(string.format(">>> [FLAG] 0x%04X is already %s.", fid, on and "set" or "clear"))
        return
    end

    mainmemory.write_u8(addr, want and (byte + mask) or (byte - mask))
    print(string.format(">>> [FLAG] 0x%04X %s -> %s%s", fid,
        on and "SET" or "clear", want and "SET" or "clear",
        name and ("   -- " .. name) or ""))
    if want and name then
        print(">>>        That flag is a location check. The client will report it.")
    end
end
...
local function badge_command(line)
    ...
    elseif verb == "reset" then
        ...
        print(">>>   diag               full diagnostic dump -- use this if checks never fire")
        print(">>>   flag 0x277         report an event flag")
        print(">>>   flag 0x277 on|off  set or clear one, for working out what gates what")
...
-- There used to be an `ap_granted_items` table here, filled by watching
-- Archipelago's bag writes, so a manual claim could tell "you already had this
-- from the multiworld" from "you just picked up the vanilla one".
--
-- It is gone on purpose. It could only ever see writes made while this script
-- was running, so reloading the connector mid-run made it forget and delete a
-- copy it should have kept. The Archipelago client makes that call now: the
-- server resends the full received-items list on every connect, so the client
-- knows across sessions. See MANUAL_CHECK_ITEMS in Client.py.
...
local function manual_claim(idx)
    ...
    -- Removing the vanilla copy is deliberately NOT done here any more.
    --
    -- Whether your copy should be taken away depends on whether Archipelago has
    -- already granted you that item, and this script cannot know that reliably:
    -- it could only watch writes made while it happened to be running, so a
    -- reload mid-run made it forget and delete a copy it should have kept.
    --
    -- The Archipelago client knows, and knows it across sessions, because the
    -- server resends the full received-items list on every connect. So the
    -- button's only job is to set the flag; the client sees the check fire and
    -- handles the bag itself.
    event_flag_set(e.flag)
    print(string.format(">>> [MANUAL CHECK SENT] %s (%s, flag 0x%04X).", e.name, e.where, e.flag))
    print(">>>          The Archipelago client will report it on its next poll, and")
    print(">>>          will take the vanilla copy out of your bag unless it had")
    print(">>>          already granted you that item.")
end

-- ============================================================================
-- BLOCKER OVERRIDES  --  safety valve
--
-- The Archipelago client puts story roadblocks back until the multiworld grants
-- the item that clears them. That can strand you: Snorlax respawns on Route 7
-- when the area reloads, and if you are on the far side there is no way back.
-- Being stuck is a softlock, which is worse than the sequence break the blocker
-- prevents, so there has to be a way out.
--
-- Setting the override flag tells the client to stop managing that roadblock
-- entirely. It is a real event flag, so it survives saving and reconnecting. The
-- blocker stays off until you turn it back on.
-- ============================================================================
local BLOCKERS = {
    { name = "Snorlax (Route 7)", flag = 0x0277, override = 0x0013 },
}
local blocker_buttons = {}

local function event_flag_clear(flag)
    local addr = EVENT_FLAGS_BASE + math.floor(flag / 8)
    local byte = mainmemory.read_u8(addr)
    local mask = BADGE_MASK[flag % 8]
...
-- Turn the client's management of a roadblock on or off.
local function blocker_toggle(idx)
    local b = BLOCKERS[idx]
    if event_flag_is_set(b.override) then
        event_flag_clear(b.override)
        print(string.format(">>> [BLOCKER] %s is managed by Archipelago again.", b.name))
        print(">>>           It will be put back until the multiworld grants the item.")
    else
        event_flag_set(b.override)
        print(string.format(">>> [BLOCKER] %s is now left alone by Archipelago.", b.name))
        print(">>>           Use the Clear button if it is currently in your way.")
    end
end

-- Remove the obstacle right now. Only sticks while the override is on, since
-- otherwise the client will put it straight back.
local function blocker_clear_now(idx)
    local b = BLOCKERS[idx]
    if not event_flag_is_set(b.override) then
        event_flag_set(b.override)
        print(string.format(">>> [BLOCKER] Switched off Archipelago's management of %s first,", b.name))
        print(">>>           otherwise it would be restored within a second.")
    end
    event_flag_set(b.flag)
    print(string.format(">>> [BLOCKER] %s cleared. Leave the area and come back to see it --", b.name))
...
local function badge_init_form()
    local ok, err = pcall(function()
        ...
        -- is one instruction too many.
        forms.button(badge_form, "Diagnostics", function()
            connector_diagnostics()
        end, 350, 6, 140, 24)
        ...
        local by = my + 38 + #MANUAL_ITEMS * 26 + 6
        forms.label(badge_form, "Story roadblocks:", 10, by, 140, 16)
        for i, b in ipairs(BLOCKERS) do
            local y = by + 20 + (i - 1) * 26
            forms.label(badge_form, b.name, 10, y + 4, 120, 16)
            blocker_buttons[i] = forms.button(badge_form, "...", function()
                blocker_toggle(i)
            end, 134, y, 150, 22)
            forms.button(badge_form, "Clear it now", function()
                blocker_clear_now(i)
            end, 290, y, 100, 22)
        end

        local wy = by + 20 + #BLOCKERS * 26 + 4
        forms.label(badge_form,
...
local function badge_tick()
    ...
    -- Roadblock buttons: show whether Archipelago is managing each one.
    for i, b in ipairs(BLOCKERS) do
        if blocker_buttons[i] then
            local caption = event_flag_is_set(b.override)
                and "Archipelago: OFF" or "Archipelago: managing"
            local ok, cur = pcall(forms.gettext, blocker_buttons[i])
            if ok and cur ~= caption then
                pcall(forms.settext, blocker_buttons[i], caption)
            end
        end
    end
...
local request_handlers = {
    ...
    ["HASH"] = function(req)
        -- Report the hash AND the ROM name, separated by " | ".
        --
        -- The hash alone is not a dependable identity: BizHawk returns "N/A" for
        -- some dump formats (a known-good Pokemon X dump does exactly that) while
        -- hashing others fine. The name has been present and meaningful in every
        -- dump seen so far, and always contains "Pokemon X" or "Pokemon Y" --
        -- whether it comes from BizHawk's game database or from the file header.
        --
        -- The client parses this apart; a client that does not understand the
        -- format just sees a longer string, which is harmless.
        -- The leading marker identifies THIS connector, which matters because
        -- several Archipelago worlds can target the 3DS. The client asks every
        -- installed world "is this your game?", and without a marker the Pokemon
        -- X/Y world would happily claim, say, A Link Between Worlds: that game is
        -- also 3DS, and its own connector sends no ROM name for us to reject.
        local hash = "NOHASH"
        if gameinfo and gameinfo.getromhash then
            local h = gameinfo.getromhash()
            if h and h ~= "" and h ~= "N/A" then hash = h end
        end

        local name = ""
        if gameinfo and gameinfo.getromname then
            name = gameinfo.getromname() or ""
        end

        return {type = "HASH_RESPONSE", value = "PKMNXY|" .. hash .. "|" .. name}
    end,
    ["GUARD"] = function(req)
...
-- SAVESTATE HANDLING
--
-- Loading a savestate swaps the entire game state in a single frame, which makes
-- every baseline this script holds instantly wrong. Left alone that is actively
-- destructive, not merely noisy: hundreds of event flags appear to flip 0 -> 1
-- at once, each one arming the vanilla-item interceptor, which then spends the
-- next ten seconds deleting real items out of the bag looking for pickups that
-- never happened.
--
-- So on a state load every baseline is thrown away and rebuilt. Held Archipelago
-- writes are RELEASED rather than discarded, so no incoming item is lost.
-- ============================================================================
local function rebaseline(reason)
    -- Deliver anything the interceptor was holding before clearing its state,
    -- otherwise those items vanish.
    release_held_writes()
    ...
    -- `checked` only suppresses duplicate console lines, so clearing it just
    -- makes the log honest about the new timeline.
    checked          = {}
    last_bag         = take_bag_snapshot()
    badge_last_field = -1

    print(string.format(">>> [REBASELINE] %s -- flag and bag baselines reset.", reason))
end

-- The reliable signal, where the build offers it.
local have_loadstate_event = pcall(function()
    event.onloadstate(function() rebaseline("savestate loaded") end)
end)
if not have_loadstate_event then
    print(">>> event.onloadstate unavailable; falling back to frame-counter detection.")
end

local last_framecount = -1
...
while true do
    -- Fallback state-load detection for builds without event.onloadstate. The
    -- frame counter running backwards can only mean a state was loaded. A
    -- forwards jump is deliberately NOT treated as one, because fast-forward and
    -- frame skip produce those legitimately.
    local fc = emu.framecount()
    if last_framecount >= 0 and fc < last_framecount then
        rebaseline("frame counter went backwards")
    end
    last_framecount = fc
